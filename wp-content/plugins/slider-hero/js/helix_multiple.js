...
if (typeof helix_multiple_mainId === 'undefined' || helix_multiple_mainId === null) {
    helix_multiple_mainId = mainId;
}
...
var mainArea = document.getElementById(helix_multiple_mainId);
...
function setup() {
	...
	helix = stage.addChild(new Helix(10));
	helix.x = w/2;
	helix.y = h/2;
	helix.speed = 0.1;
	helix.alpha = 0.1;
	...
	helix = stage.addChild(helix.clone(50));
...
function onResize() {
	w = $('#'+helix_multiple_mainId).width();
	h = $('#'+helix_multiple_mainId).height();
	max = Math.max(w,h);
	min = Math.min(w,h);
	target.width = w;
	target.height = h;
