...
if (typeof chaos_mainId === 'undefined' || chaos_mainId === null) {
    chaos_mainId = mainId;
}

var mainArea = document.getElementById(chaos_mainId);
...
function setup() {
	...
	for (var i=0; i<7; i++) {
		var helix = stage.addChild(new Helix(i*50+100));
		helix.x = w/2;
		helix.y = h/2;
		helix.particleScale = 200/helix.particleCount*0.5;
...
function onResize() {
	w = $('#'+chaos_mainId).width();
	h = $('#'+chaos_mainId).height();
	max = Math.max(w,h);
	min = Math.min(w,h);
	target.width = w;
	target.height = h;
