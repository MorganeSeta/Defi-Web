...
if(jQuery('#trails-canvas').length>0){
	
if (typeof firework_mainId === 'undefined' || firework_mainId === null) {
    firework_mainId = mainId;
}
"use strict";
console.clear();
...
const appNodes = {
	stageContainer: "#"+firework_mainId,
	canvasContainer: "#hero-canvas-container",
	controls: "#controls",
	menu: "#menu",
	pauseBtn: "#pause-btn",
...
function renderApp(state) {
	...
	//appNodes.skyLighting.checked = state.config.skyLighting;
...
function getConfigFromDOM() {
	return {
		...
		skyLighting: appNodes.skyLighting.checked,
...
function seqSmallBarrage() {
	...
	// (cos(x*5p+0.5p)+1)/2 is a custom wave bounded by 0 and 1 used to set varying launch heights
	function launchShell(x) {
		const isRandom = shellNameSelector() === "Random";
		let shellType = isRandom
			? useCrysanthemum ? crysanthemumShell : randomFastShell()
...
function handleResize() {
	const w = $('#'+mainId).width();
	const h = $('#'+mainId).height();
...
function render(speed) {
	...
	if (store.state.config.skyLighting) {
		colorSky(speed);
	}
	...
	trailsCtx.fillStyle = `rgba(0, 0, 0, ${store.state.longExposure
		? 0.0025
		: 0.1 * speed})`;
	...
	while (BurstFlash.active.length) {
		...
		burstGradient.addColorStop(0.024, "white");
...
function colorSky(speed) {
	...
	appNodes.canvasContainer.style.backgroundColor = `rgb(${currentSkyColor.r |
		0}, ${currentSkyColor.g | 0}, ${currentSkyColor.b | 0})`;
}
