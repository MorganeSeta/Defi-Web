...
if (typeof antigravity_mainId === 'undefined' || antigravity_mainId === null) {
    antigravity_mainId = mainId;
}
...
var Particle = function () {
  ...
  Particle.prototype.render = function render() {
    ...
    $('#'+antigravity_mainId).append(this.item);
    return {
...
setInterval(function () {
  if (!isPaused) {
    particles.push(new Particle(data[randomInt(0, data.length - 1)], {
      "x": Math.random() * $('#'+antigravity_mainId).width(),
      "y": $('#'+antigravity_mainId).height()
    }, 1 + Math.random() * 3));
  }
}, 200);
